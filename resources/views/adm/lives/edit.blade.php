		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-lg-12 col-md-12 col-12">
					...
					<!-- Page header -->
					<div class="mb-5">
						<h3 class="mb-0 ">Éditer Live/Vidéo</h3>
					</div>
				</div>
			</div>
			<div>
            <form action="{{route('admin.updatelives',['id'=>$find->id])}}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
				<!-- row -->
				<div class="row">
							...
							<div class="card-body">
								<div>
									...
									<div class="mb-3  {{ $errors->has('titre') ? ' has-error' : '' }}">
										...
										<input type="text" name="titre" class="form-control" value="{{$find->titre}}" placeholder="Entrez l'intitulé de la vidéo" >
									    @if ($errors->has('titre'))
									...
									<div class="mb-3  {{ $errors->has('lien') ? ' has-error' : '' }}">
										...
										<input type="text" name="lien" class="form-control" value="{{$find->lien}}" placeholder="Lien vers la vidéo" >
									    @if ($errors->has('lien'))
                                            <div class="alert" role="alert">
									...
									<div class="mb-4   {{ $errors->has('artiste') ? ' has-error' : '' }}">
										...
										<select name="artiste" id="artiste" class="form-control">
											...
											@forelse ($artistes as $artiste)
												<option value="{{$artiste->id}}" {{$find->user_id==$artiste->id ? 'selected' : ''}}>{{$artiste->nom}}</option>
							...
							<div class="card-body">
								<div>
									<div class="mb-4   {{ $errors->has('banniere') ? ' has-error' : '' }}">
										...
										<p>
											Sélectionnez une miniature sous un format 500px x 340px si vous voulez remplacer l'ancienne
										</p>
										<!-- input -->
									...
									<div>
										...
										<div class="d-block dropzone border-dashed rounded-2 text-center">
											<img src="{{asset('PlayerTemplate/img/live/'.$find->banniere)}}" id="blah1" class="img-fluid" alt="">
										</div>
									</div>
								</div>
							</div>
								...
								<div>
									...
									<div class="mb-3  {{ $errors->has('en_cours') ? ' has-error' : '' }}">
										...
										<select class="form-select" name="en_cours" aria-label="Default select example">
											<option  value="1" {{$find->encours==1 ? 'selected' : ''}}  >Oui</option>
											<option  value="0" {{$find->encours==0 ? 'selected' : ''}}  >Non</option>
										</select>
