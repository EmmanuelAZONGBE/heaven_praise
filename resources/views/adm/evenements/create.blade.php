            ...
            <form action="{{route('admin.storeevents')}}" method="post" enctype="multipart/form-data">
				...
				<div class="row">
							...
							<div class="card-body">
								<div>
									...
									<div class="mb-3  {{ $errors->has('titre') ? ' has-error' : '' }}">
										...
										<input type="text" name="titre" class="form-control" value="{{old('titre')}}" placeholder="Entrez l'intitulé de l'évènement" >
									    @if ($errors->has('titre'))
									...
									<div class="mb-3  {{ $errors->has('slug') ? ' has-error' : '' }}">
										...
										<input type="text" name="slug" class="form-control" value="{{old('slug')}}" placeholder="Exemple : nom-de-l-evenement" >
									    @if ($errors->has('slug'))
                                            <div class="alert" role="alert">
									...
									<div class="  {{ $errors->has('description') ? ' has-error' : '' }}">
										...
										<textarea  class="form-control" name="description" id="texteditor" cols="30" rows="10">{!!old('description')!!}
                                        </textarea>
					...
					<div class="col-lg-4 col-12">
						...
						<div class="card mb-4">
							...
							<div class="card-body">
								...
								<div class="form-check form-switch mb-4">
									<input class="form-check-input" name="billeterie" type="checkbox" role="switch" id="flexSwitchStock" {{!empty(old('billeterie')) ? 'checked' : ''}} >
								...
								<div>
									...
									<div class="mb-3  {{ $errors->has('statut') ? ' has-error' : '' }}">
										...
										<select class="form-select" name="statut" aria-label="Default select example">
											...
											<option  value="Non Publié"  {{old('statut')=="Non Publié" ? 'selected' : ''}} >Non Publié</option>
										</select>
									...
									<div class="mb-3  {{ $errors->has('pays') ? ' has-error' : '' }}">
										...
										<select class="form-select" name="categorie" aria-label="Default select example">
											@forelse ($categories as $categorie)
												<option  value="{{$categorie->id}}" {{old('categorie')==$categorie->id ? 'selected' : ''}} >{{$categorie->libelle}}</option>
						...
						<div class="card mb-4">
							...
							<div class="card-body">
								...
								<div class="mb-3 {{ $errors->has('lieu') ? ' has-error' : '' }}">
									...
									<input type="text" class="form-control" name="lieu" value="{{old('lieu')}}" placeholder="Où se déroulera l'évènement?">
                                    @if ($errors->has('lieu'))
                                        <div class="alert" role="alert">
								...
								<div class="mb-3 {{ $errors->has('date') ? ' has-error' : '' }}">
                                    ...
                                    <input class="form-control " type="date" name="date" value="{{old('date')}}" placeholder="Selectionner la Date"aria-describedby="basic-addon2">
                                    @if ($errors->has('date'))
                                        <div class="alert" role="alert">
								...
								<div class="mb-3  {{ $errors->has('pays') ? ' has-error' : '' }}">
									...
									<select class="form-select" name="pays" aria-label="Default select example">
										@forelse ($pays as $pay)
											<option  value="{{$pay->id}}" {{old('pays')==$pay->id ? 'selected' : ''}} >{{$pay->libelle}}</option>
						...
						<!-- button -->
						<div class="d-grid">
							<button type="submit" class="btn btn-primary">Creer l'Évènement</button>
						</div>
					</div>
				</div>
            </form>    
